...
int main(int argc, char* argv[])
{
	...
	clock_t start, end,pre,ffph,duiqi; //long
	start = clock();//开始时间
	pointcloud::Ptr source(new pointcloud);
	pointcloud::Ptr target(new pointcloud);
	pcl::io::loadPCDFile("out0.pcd", *target);	pcl::io::loadPCDFile("outzhuan.pcd", *source);
	...
	sac_ia.setCorrespondenceRandomness(4);//设置计算协方差时选择多少近邻点，该值越大，协防差越精确，但是计算效率越低.(可省)
	...
	cout << "预处理 " << float(pre - start) / CLOCKS_PER_SEC << endl;
	cout << "ffph " << float(ffph - pre) / CLOCKS_PER_SEC << endl;
	cout << "ffph2 " << float(duiqi - ffph) / CLOCKS_PER_SEC << endl;
	cout << "对齐 " << float(end - duiqi) / CLOCKS_PER_SEC << endl;
